...
import { INTERVIEW_WIZARD_STEPS } from "../../aiInterview/constants/navigation";
import { INTERVIEW_QUESTIONS } from "../../aiInterview/data/mockData";
import {
  Button,
  InterviewPrepOverlay,
  PostInterviewProcessingScreen,
  ThemeToggleButton,
} from "../../../components/ui";
import { findJobPostingById, JOB_POSTINGS } from "../../aiInterview/data/mockData";
import { formatTime } from "../../aiInterview/utils/formatters";
...
const INTERVIEW_ROUNDS = [
  {
    title: "Vòng 1",
    name: "HR screening",
    detail: "Giới thiệu bản thân, động lực ứng tuyển và kỳ vọng công việc.",
  },
  {
    title: "Vòng 2",
    name: "Technical fit",
    detail: "Kinh nghiệm, kỹ năng chính và mức độ phù hợp với JD.",
  },
  {
    title: "Vòng 3",
    name: "Situation case",
    detail: "Câu hỏi tình huống, cách xử lý vấn đề và giao tiếp.",
  },
  {
    title: "Vòng 4",
    name: "Wrap-up",
    detail: "Tổng kết, câu hỏi ngược và khuyến nghị luyện tập tiếp theo.",
  },
];
...
function buildInterviewTranscript(chatMessages = []) {
  ...
  messages.forEach((message) => {
    if (message?.sender === "ai") {
      currentQuestion = message.text || currentQuestion;
      return;
    }
    if (message?.sender === "user") {
      pairs.push({
        question: currentQuestion || "Câu hỏi phỏng vấn",
        answer: message.text || "",
        score: Math.max(62, Math.min(96, 78 + pairs.length * 4)),
        feedback:
          pairs.length % 2 === 0
            ? "Câu trả lời có cấu trúc tốt, nên bổ sung thêm số liệu impact."
            : "Nội dung bám JD, cần rút gọn phần mở đầu để đi thẳng vào kết quả.",
      });
    }
  });
  ...
  return INTERVIEW_QUESTIONS.slice(0, 3).map((question, index) => ({
    question,
    answer: "Ứng viên chưa có transcript đầy đủ cho câu hỏi này.",
    score: 76 + index * 3,
    feedback: "Cần hoàn thiện dữ liệu transcript để AI phân tích sâu hơn.",
  }));
}
...
function LiveStepScreen({
  ...
  const answeredCount = Array.isArray(chatMessages) ? chatMessages.filter((message) => message?.sender === "user").length : 0;
  const currentRoundIndex = Math.min(INTERVIEW_ROUNDS.length - 1, Math.floor(answeredCount / 2));
  ...
  const handleRecordClick = () => {
    ...
    if (isRecording) {
      setIsRecording(false);
      const safe = (transcript || "").trim() || "Câu trả lời đã ghi âm.";
      onSubmitAnswer(safe);
      return;
    }
    setIsRecording(true);
  ...
  return (
    <section className={`interview-live-studio min-h-screen ${bgShell}`}>
      <div className="mx-auto w-full max-w-none pl-6 pr-0 py-4">
        ...
        <div className="live-round-strip">
          {INTERVIEW_ROUNDS.map((round, index) => (
            <article
              key={round.title}
              className={`live-round-card ${index < currentRoundIndex ? "done" : ""} ${index === currentRoundIndex ? "active" : ""}`}
            >
        ...
        <div className="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-12">
          ...
          <aside className="lg:col-span-7">
            <div className={`live-chat-shell rounded-l-2xl rounded-r-none border p-5 shadow-sm ${panel}`}>
...
export default function InterviewWizardPage() {
  ...
  const cvOptions = useMemo(() => {
    ...
    const merged = rows.length ? rows : fallbackRows;
  ...
  const startResultProcessing = () => {
    ...
    writeLastInterviewResult({
      ...
      transcript: buildInterviewTranscript(chatMessages),
      summary: {
        strengths: [
          "Trả lời có cấu trúc rõ ràng và bám sát JD.",
          "Thể hiện được kinh nghiệm phối hợp với Security và Engineering.",
        ],
        improvements: [
          "Cần định lượng impact bằng KPI/KRI hoặc tỷ lệ cải thiện.",
          "Nên trả lời gọn hơn trong các câu hỏi tình huống.",
        ],
        recommendation: "Nên luyện thêm bộ câu hỏi technical và tình huống áp lực cao trước vòng tiếp theo.",
      },
      components: {
        jdMatch,
        logic,
  ...
  const renderStep = () => {
    if (step === 0) {
      ...
      return (
        <section className="wizard-card wizard-card-lg studio-setup-card">
          <div className="wizard-card-head studio-card-head">
            <span className="studio-kicker"><Sparkles className="h-4 w-4" /> Interview setup</span>
            <h1>Thiết kế phiên phỏng vấn AI</h1>
            <p>Chọn CV và JD để AI tạo bối cảnh phỏng vấn đúng vị trí, đúng năng lực cần đánh giá.</p>
          </div>

          <div className="wizard-grid two-col wizard-upload-grid">
            <section className="wizard-block studio-input-panel">
            ...
            <section className="wizard-block studio-input-panel">
	          ...
	          <section className="interview-round-plan">
	            <div className="interview-round-plan-head">
	              <span className="studio-kicker"><CheckCircle2 className="h-4 w-4" /> Interview rounds</span>
	              <h2>Quy trình phỏng vấn mô phỏng 4 vòng</h2>
	              <p>AI sẽ dẫn dắt phiên theo cấu trúc gần với phỏng vấn thật: sàng lọc, chuyên môn, tình huống và tổng kết.</p>
	            </div>
	            <div className="interview-round-grid">
	              {INTERVIEW_ROUNDS.map((round) => (
	                <article key={round.title}>
	                  <span>{round.title}</span>
	                  <strong>{round.name}</strong>
	                  <p>{round.detail}</p>
	                </article>
    ...
    if (step === 1) {
      return (
        <section className="wizard-card wizard-card-lg studio-device-card">
          <div className="wizard-card-head studio-card-head">
            <span className="studio-kicker"><Gauge className="h-4 w-4" /> Device check</span>
            <h1>Kiểm tra phòng luyện tập</h1>
            <p>Đảm bảo camera, micro và môi trường trả lời ổn định trước khi bắt đầu.</p>
          </div>
          ...
          <div className="wizard-device-layout">
            <section className="wizard-preview-card">
              ...
              <div className={`wizard-preview ${cameraOn ? "" : "off"}`}>
                ...
                {cameraOn && !userStream ? <div className="wizard-preview-placeholder subtle">Đang mở camera</div> : null}
              </div>

              <div className="wizard-audio-head">
                <strong>Kiểm tra micro</strong>
  ...
  const sessionStatus = isProcessingResult
    ? { label: "Analyzing", className: "status-badge-warning" }
    : step >= 3
      ? { label: "In progress", className: "status-badge-primary" }
      : step === 2
        ? { label: isPreparingInterview ? "Preparing" : "Not started", className: "status-badge-warning" }
        : { label: "Not started", className: "status-badge-neutral" };

  return (
    <main className={`interview-legacy interview-wizard interview-studio theme-${theme} ${step >= 2 ? "immersive" : ""}`}>
         <header className={`wizard-stepper-wrap ${step >= 2 ? "floating" : ""} relative`}>
           ...
           <div className="absolute right-4 top-1/2 z-40 flex -translate-y-1/2 items-center gap-2">
             <span className={`status-badge ${sessionStatus.className}`}>{sessionStatus.label}</span>
